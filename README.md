...
- in Rule Designer, you need to generate the XOM (and the Ruleapp):
    ...
    - right click on the 'deployument/deployer' file in the vehicle-lifecycle-decision-service project explorer and select 'Rule Execution Server / Deploy...'
...
- enter: 'npm run deployXom ../vehicle-lifecycle-decision-service/resources/xom-libraries/vehicle-lifecycle-xom.zip 1.0'
...
In the previous step, you should have generated the ruleapp supporting the vehicle lifecycle decision service using Rule Designer. The ruleapp is the 'vehicle.jar' generated in the output directory. 
...
Refer to the README in 'vehicle-lifecycle-cli' directory to initialize the application and run a demo scenario that illustrate the application of the business rules when suspicious transactions are generated. 
...
4/ Deploy vehicle-lifecycle Business Network
    ...
    - 'npm run deploy'
5/ Deploy the XOM via Blockchain (provided the XOM has been built by Rule Designer before)
    - go to 'vehicle-lifecycle-cli'
    - the first time you need to run 'npm install' to set-up the client application
    - 'npm run deployXom'
6/ Deploy the Ruleapp via Blockchain (provided the Ruleapp has been built by Rule Designer before)
...
using the following command from vehicle-lifecycle-cli: 'npm run demo'
