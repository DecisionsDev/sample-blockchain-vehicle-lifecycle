...
3/ Deploy vehicle-lifecycle Business Network
- go to `vehicle-lifecycle`
- `npm run deploy`

4/ Deploy the XOM via Blockchain (provided the XOM has been generated by Rule Designer before)
- go to `vehicle-lifecycle-cli`
- the first time you need to run `npm install` to set-up the client application
- `npm run deployXom`

5/ Deploy the Ruleapp via Blockchain (provided the Ruleapp has been generated by Rule Designer before)
- `npm run deployRuleapp`

6/ Setup the data of the Business Network
- `npm run setup ; npm run listVehicles`

7/ Run the suspicious transactions
- `npm run transfers`
